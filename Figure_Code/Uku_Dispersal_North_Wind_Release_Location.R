...
Mean_V_Wind_Annual<-Wind_Data_Annual[21:33,4]
...
for (ii in 1:nrow(Corr_Coeff_Maui_Nui_Retention)){
  ...
  Resp1<-t(Resp)
  DF<-as.data.frame(cbind(Resp1, Mean_V_Wind_Annual))
  names(DF)[1] <- 'Retention'
  X<-cor.test(DF$Mean_V_Wind_Annual, DF$Retention)
...
Release_Locations_Sig_Maui_Nui<-Release_Locations_Maui_Nui[Release_Locations_Maui_Nui$Sig<=0.05 & Release_Locations_Maui_Nui$Mean>=0.01,]
  ...
  png(paste0("Correlation_with_North_Wind_Strength_",Islands[i],".png"), height=5, width=7, units="in", res=300)
